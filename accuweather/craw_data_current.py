...
# scrapy runspider craw_data_current.py -o weather_info_today_mis.json
...
weather_df['link']=weather_df['link'].apply(lambda x:x.replace('march',month_num_name[int(pd.datetime.now().month-1)])) #february
...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for quote in response.css('a.is-today'):
            res = quote.xpath('div//text()').extract()

            res = [res[0]]+ [res[4]] + [res[6]]+ [res[9]] + [res[11]]
